...
export const Card = ({
  ...
  return (
    <div
      className={`transition-[max-height] duration-200 ease-in ${
        openMore
          ? "md:max-h-[18rem] min-[540px]:max-h-[10.5rem] min-[717px]:max-h-[11.5rem]  max-h-[9.2rem]"
          : "lg:max-h-[12.3rem] md:max-h-[11.5rem] min-[540px]:max-h-[7.5rem] min-[717px]:max-h-[8.5rem] max-h-[6.2rem]"
      } w-full relative border mt-2 text-center bg-[#004346] text-white rounded-xl  overflow-hidden lg:pt-8 md:pt-6 pt-3`}>
      <div className={`flex flex-row ${openMore ? "pb-3" : "pb-5"}`}>
        <p className="md:text-xl text-lg font-bold w-1/4 my-auto">
          {time}
        </p>
        ...
        <div className="flex flex-col w-1/4">
          <img
            src={src}
            className="w-[40%] lg:w-[30%] mx-auto"
          />
      ...
      <BsChevronDoubleDown
        ...
        className={`cursor-pointer absolute lg:bottom-3 bottom-2 lg:left-[49%] lg:right-[49%] lg:w-[2%] w-[3.5%] left-[48.25%] ${
          openMore ? "rotate-180" : "z-10"
        }`}
      />

      <div
        id="extra"
        className={`flex flex-row mb-10 ${openMore ? "" : "opacity-0"}`}>
        <span className="w-1/3 flex flex-col lg:block text-center text-xs md:text-base">
          ...
          <span className="order-1">{windSpeed}km/h</span>
        </span>
        ...
        <span className="w-1/3 flex flex-col lg:block text-center text-xs md:text-base">
          ...
          <span className="order-1">{humidity}%</span>
        </span>
      </div>
    </div>
  );
